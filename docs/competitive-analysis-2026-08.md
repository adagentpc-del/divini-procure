...
8. **AI-generated scope-of-work / RFP bundles from plans and specs** — Brickanta, Parspec, Procore AI. A live 2025-26 competitive wedge; strong structural fit for Divini's canonical data spine, but explicitly out of scope under the current AI freeze ("AI package generation" is named directly). **Closed (AI-01, 2026-08-17, freeze explicitly opened by the user together with #5):** `POST /scope/instances/:id/ai-draft` (`server/src/routes/scope-builder.ts`) drafts the standard scope narrative fields (site conditions, access restrictions, delivery/install requirements, exclusions, acceptance criteria) grounded strictly in real extracted document text (`documents.extracted_text` - schema-blueprint-content-extraction.sql's PDF-text-layer/OCR/DXF extraction, sanitized through `sanitizeForLlm()` before ever reaching a prompt), for documents linked to the scope's package either directly or via `blueprint_document_package_links`. Never writes to `scope_instances` - returns the draft only; `ScopeBuilder.tsx` pre-fills the already-editable form fields (only ones currently empty, never overwriting existing user text) and the user reviews and explicitly clicks the pre-existing Save button, same trust model as every other AI touch point in this codebase (`docs/ai-layer-design.md` §1, `docs/AI-GUARDRAILS.md`). Degrades gracefully with a clear reason whenever the LLM isn't configured, no package is linked, or no analyzable document content exists - never a hard error, matching `lib/llm.ts`'s "never a hard dependency" contract. This also closes the honest half of the gap Divini Blueprint's own `blueprint_trade_suggestions` -> `create-scope` flow left open (an empty scope form) since before this session, per the freeze-opening section below.
...
- **#8 closed (AI-01)** - see its updated entry above. Scoping found
  Divini Blueprint already had most of the needed infrastructure
  (`ai_extraction_runs`, `blueprint_trade_suggestions`, real document text
  extraction via `documents.extracted_text` -
  ...
  drafted content at all - the user started from a blank Divini Scope
  Builder form. AI-01 closes (b) by drafting the standard scope narrative
  fields from real extracted document text, never auto-saved - the user
  reviews and explicitly saves, same trust model as every other AI touch
  point in this codebase. (a) stays open - `draftAiSummaryFields()`
  itself is untouched - since it's a narrower, lower-value gap now that
  AI-01 gives the higher-value path (drafting real scope content) its own
  grounded route.

**Status as of this freeze-opening:** #5 (QCB-01) and #8 (AI-01) both
closed. #9 (pricing/trade-credit) and #14 (investor/Capital Partner)
remain frozen - not opened, not touched. With this, every gap across
both competitive-analysis passes that can be honestly built without
fabricating data or crossing a still-standing freeze has shipped.
