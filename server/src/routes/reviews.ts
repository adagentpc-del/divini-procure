...
router.get(
  ...
  h(async (req, res) => {
    ...
    const rows = await q<any>(
      `select r.*, rc.name as rater_name
         from reviews r
    ...
    const stats = await q1<{ avg: number | null; cnt: string }>(
      `select avg(stars) as avg, count(*) as cnt from reviews r where ${conditions.join(" and ")}`,
      params,
    );
    res.json({
      reviews: rows,
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const { packageId, vendorCompanyId, stars, body } = (req.body ?? {}) as Record<string, unknown>;
    ...
    const row = await q1<any>(
      `insert into reviews (rater_company_id, ratee_company_id, package_id, stars, body)
       values ($1,$2,$3,$4,$5)
       returning *`,
      [raterCompanyId, vendorCompanyId, packageId, starsNum, typeof body === "string" ? body.slice(0, 4000) : null],
    );
...
router.patch(
  ...
  h(async (req, res) => {
    ...
    const { stars, body } = (req.body ?? {}) as Record<string, unknown>;
    const sets: string[] = [];
    const params: unknown[] = [];
